...
# ВАЖНО: скорость зависит не только от языка, но и от движка. Piper тараторит,
# Qwen говорит спокойнее примерно на треть. Значения ниже замерены по готовым
# роликам, а не по прикидке: китайский 3.9 вместо 5.8, немецкий 15.8 вместо
# 17.4. С прежними цифрами китайский вышел на 29-34 секунды вместо двадцати.
# rf стоит 12.2, а не 14.5: он идёт на Qwen, а тот говорит медленнее Piper.
SKOROST = {"zh": 3.9, "ja": 4.5, "ko": 5.5,
           "ru": 14.5, "rf": 12.2, "kk": 15.2, "uk": 13.9, "uz": 14.5, "tr": 15.5,
           "en": 17.7, "de": 15.8, "fr": 17.8, "es": 15.5, "it": 15.5}
...
def frazy(t):
    """Разбить текст на фразы.

    В китайском и японском предложения разделяются своими знаками и БЕЗ пробела
    после них. Прежнее правило требовало пробел, поэтому весь китайский текст
    считался одной фразой: резать было нечего, и ролик выходил на 32 секунды
    сплошным куском вместо двадцати с паузами.
    """
    return [f.strip() for f in re.split(r"(?<=[.!?])\s+|(?<=[。！？；])", (t or "").strip())
            if f.strip()]
...
def korotkiy(card, lang, cta):
    ...
    # Запас считаем в СЕКУНДАХ, а не в знаках. Двадцать знаков запаса для
    # английского это секунда, а для китайского три с половиной: с общей
    # меркой китайский ролик схлопывался до одиннадцати секунд.
    zapas = int(SKOROST.get(lang, 14.5) * 1.2)
    mesto = predel - zapas - (len(frazy(cta)[0]) if cta and frazy(cta) else 0)
    for f in ff[1:]:
        if f in out:
            continue
