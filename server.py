...
def opisaniya(week, slot, lang):
    """Задания для модели берём из promt.py - там жёсткая структура на английском.
    Пересказ киносценария модель понимает буквально и рисует камеру в кадре."""
    import promt
    return promt.dlya_nedeli(week, slot, lang)
...
def cmd_rolik(week, slot, lang):
    ...
    # ПРОВЕРКА ПЕРЕД СБОРКОЙ. Один раз ролик уже собрался на стоке, хотя клипы
    # были куплены и лежали рядом - деньги ушли впустую. Больше не запускаем
    # сборку вслепую: сначала убеждаемся, что каждый оплаченный клип на месте.
    est = [f"{U}/{week}{slot}-{i}-{cast}.mp4" for i in range(len(pr))]
    net = [p for p in est if not os.path.exists(p)]
    if net:
        raise SystemExit("СТОП: не готовы клипы, сборка была бы на стоке:\n  "
                         + "\n  ".join(os.path.basename(x) for x in net))
    print(f"  клипов к сборке: {len(est)}, все на месте", flush=True)
    ...
    # Считаем, сколько планов реально взято из оплаченного, и говорим вслух.
    use = r.stdout.count("живое видео:")
    sto = r.stdout.count("материала") - use - r.stdout.count("кадр:")
    print(r.stdout[-400:] if "DONE" not in r.stdout else
          [l for l in r.stdout.splitlines() if "DONE" in l][0])
    print(f"  планов из купленного: {use}, со стока: {max(0,sto)}")
    if use == 0:
        print("  ВНИМАНИЕ: оплаченные клипы не попали в ролик")
